...
const withAndroidWidgetFiles = (config, widgets) => {
    return (0, config_plugins_1.withDangerousMod)(config, [
        ...
        async (config) => {
            ...
            fs.writeFileSync(path.join(valuesDirectory, 'expo_widgets.xml'), createWidgetStringsXml(widgets));
...
function createWidgetProviderKt(androidPackage, widget) {
...
function createWidgetInfoXml(widget) {
...
function getAndroidWidgetConfig(widget) {
    return {
...
function createWidgetStringsXml(widgets) {
    return `<?xml version="1.0" encoding="utf-8"?>
<resources>
${widgets
        .map((widget) => {
        return `  <string name="${(0, resourceNames_1.getWidgetDisplayNameResourceName)(widget)}">${escapeXmlAttribute(widget.displayName)}</string>
  <string name="${(0, resourceNames_1.getWidgetDescriptionResourceName)(widget)}">${escapeXmlAttribute(widget.description)}</string>`;
    })
        .join('\n')}
</resources>
`;
